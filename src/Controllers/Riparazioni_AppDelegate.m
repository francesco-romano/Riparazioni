...
- (IBAction)exportData:(id)sender {
    ...
    savePanel.nameFieldStringValue = @"riparazioni_export.json";
    [savePanel beginWithCompletionHandler:^(NSModalResponse result){
        if (result == NSFileHandlingPanelOKButton) {
            // Create data to be exported.
            NSError * error = nil;
            NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"Repair"];
            NSArray *allRepairs = [self.managedObjectContext executeFetchRequest:request error:&error];
            if (!allRepairs) {
                NSAlert* alert = [NSAlert alertWithMessageText:@"Failed to fetch data"
                                                 defaultButton:nil
                                               alternateButton:nil
                                                   otherButton:nil
            ...
            // Convert each object into basic types.
            NSDateFormatter* dateFormatter = [[NSDateFormatter alloc] init];
            dateFormatter.locale = [NSLocale localeWithLocaleIdentifier:@"en_US_POSIX"];
            dateFormatter.dateFormat = @"yyyy-MM-dd";
            dateFormatter.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
            NSMutableArray *exportedObjects = [[NSMutableArray alloc] initWithCapacity:allRepairs.count];
            for (Repair *repair in allRepairs) {
                NSString *position = @"";
                if (repair.position) {
                    position = repair.position.positionDescription;
                }
                
                [exportedObjects addObject:@{
                    @"date": [dateFormatter stringFromDate:repair.date],
                    @"description": repair.repairDescription ? repair.repairDescription : @"",
                    @"customer": repair.customer ? repair.customer : @"",
                    @"contact" : repair.contact ? repair.contact : @"",
                    @"notes": repair.notes ? repair.notes : @"",
                    @"position": position,
                    @"price": repair.price ? repair.price : [NSNumber numberWithInt: 0]
                }];
            }
            ...
            NSData* serializedData = [NSJSONSerialization
                                      dataWithJSONObject:exportedObjects
                                      options:NSJSONWritingPrettyPrinted
                                      error:&error];
            if (!serializedData) {
                NSAlert* alert = [NSAlert alertWithMessageText:@"Failed to export data"
                                                 defaultButton:nil
                                               alternateButton:nil
                                                   otherButton:nil
                                     informativeTextWithFormat:@"%@", error.description];
            ...
            if (![serializedData writeToURL:savePanel.URL atomically:YES]) {
                NSAlert* alert = [NSAlert alertWithMessageText:@"Failed to save data to file"
                                                 defaultButton:nil
                                               alternateButton:nil
                                                   otherButton:nil
                                     informativeTextWithFormat:@"File: %@", savePanel.URL.absoluteString];
